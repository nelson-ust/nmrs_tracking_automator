...
def tracking_obs_list(connection:str, ids:dict) -> List[Obs]:
    ...
    try:
        for patient_id, patient_data in ids.items():
                track_reason, verify_indication, track_date, who_attempt, mode_comm, person_contacted, default_reason, discontinue_care, discontinue_reason, discontinue_date, referred_service, return_date, verification_status = patient_data


            # Mapping of all the conccepts in a list.
                # concepts = [165460, 166138, 165461, 165778, 167221, 167222, 165902, 165463, 165464, 165465, 166139, 165466, 165467, 165586, 165470, 165889, 166349, 166348, 165916, 166152, 166157,165469, 165775, 165776, 165459, 165777]
                concepts = [167221, 167222, 167242, 167240, 167237, 167241, 167239, 167240, 165586, 165470, 165889, 166349, 166348, 165916, 166152, 166157, 165469, 165775, 165776, 165459, 165777]
                ...
                for concept_id in concepts:
                    ...
                    if concept_id == 166348 and get_concept_definition(discontinue_reason) != 165889: #Adult Causes
                        continue
                    if concept_id == 165916 and get_concept_definition(discontinue_reason) != 165916: #Discontinue care
                        continue
                    if concept_id == 166152 and get_concept_definition(discontinue_reason) != 5240: #Lost_to_followup
                        continue
                    if concept_id == 166157 and get_concept_definition(discontinue_reason) != 5240: #Lost_to_followup
                        continue
                    if concept_id == 165470 and discontinue_reason == '':
                        continue
                    if concept_id == 165469 and discontinue_date == '':
                    ...
                    for row in cursor:
                        ...
                        obs_info.concept_id = concept_id
                        obs_info.encounter_id = row['encounter_id']
                        obs_info.obs_datetime = (datetime.strptime(track_date, "%d/%m/%Y")).strftime("%Y-%m-%d %H:%M:%S")
                        obs_info.location_id = 14
                        obs_info.date_created = row['date_created'].strftime("%Y-%m-%d %H:%M:%S")
                        obs_info.uuid = generate_uuid()
                        ...
                        # New Concepts mapping 

                        if concept_id == 167240 and counter == 3:
                            obs_info.value_coded = 167251
                            break

                                #logic for verification Comment
                        if concept_id == 167237:
                            if get_concept_definition(verification_status) == 167243:
                                obs_info.value_text = "Pending"
                            elif get_concept_definition(verification_status) == 167244:
                                obs_info.value_text = "Patient records terminated"
                            elif get_concept_definition(verification_status) == 167245:
                                obs_info.value_text = "Client has been verified"
                            break

                        if concept_id == 167241:
                            obs_info.value_datetime = (datetime.strptime(track_date, "%d/%m/%Y")).strftime("%Y-%m-%d %H:%M:%S") #Date of Client tracking
                            break
                        if concept_id == 167239:
                            obs_info.value_coded = get_concept_definition(verification_status)
                            break
                        if concept_id == 167240 and counter == 7:
                            obs_info.value_coded = 167249
                            break
                        ...
                        if concept_id == 165469: #Discontinue Date
                            ...
                            break
                                ...
                                #Child Concepts for Death when True (165889)
                        if concept_id == 165889: #Cause of Death
                            obs_info.value_coded = 165887
                            break
                        if concept_id == 166349: #VA Cause of Death
                        ...
                        if concept_id == 166348: #Adult Causes
                            ...
                            break
                                ...
                                #Child Concepts for Discontinue care 
                        if concept_id == 165916: #Discontinue care
                            obs_info.value_coded = 165890
                            break

                                #Child Concepts for Lost_to_followup 
                        if concept_id == 166152:
                            obs_info.value_datetime = (datetime.strptime(discontinue_date, "%d/%m/%Y")).strftime("%Y-%m-%d %H:%M:%S")
                            break
                        if concept_id == 166157:
                            obs_info.value_coded = 166154
                            break
                        
                                #Concepts when client discontinuation is "No"
                        if concept_id == 165776: #Referred_service
                            obs_info.value_coded = get_concept_definition(referred_service)
                            break
                        if concept_id == 165775: #Return Date
...
def get_concept_definition(concept_name):
    ...
    try:
        concept_dcitionary = {
            ...
            'Lost_to_followup' : 5240,
            'Adherence Counseling' : 5488,
            'Verification ongoing' : 167243,
            'Patient records terminated' : 167244,
            'Records verified' : 167245,
            '1' : 167229, #No biometrics recapture
            '2' : 167224, #duplicated demographic and clinical variables
            # '3' : 167228, #Batched ART start and pickup dates
            '3' : 167235, #Same sex, DOB and ART start date
            '4' : 167223, #Consistently had drug pickup by proxy without viral load sample collection for two quarters
            # '5' : 167226, #Batched ARV pickup dates
            '5' : 167228, #Batched ART start and pickup dates
            '1&3':[167229, 167235],
            '1$4':[167229, 167223],
            '1&5':[167229, 167228],
            '2&3':[167224, 167235],
            ...
            '3&5':[167235, 167228],
            '4&5':[167223, 167228],
            '2,3&4':[167224, 167235, 167223],
            '2,3,4&5':[167224, 167235, 167223, 167228],
            '3,4&5':[167235, 167223, 167228],
...
# To update the obs table with the obs_group_id for tracking attempt
def update_verification_group_id(connection:str, ids:str): 
    ...
    try:
        for patient_id in ids:
            ...
            # To pull the group_id and person_id from the dB using nested select statement
            query = ("""SELECT max(obs_id) as group_id, person_id from obs where concept_id = 167242 
                                AND person_id in (select patient_id from patient_identifier where identifier_type = 4 and identifier = %s)""")
            ...
            for row in cursor:
                ...
                if group_id is not None:
                    ...
                    # Update of the obs records using place holders to accept value within the query statement
                    cursor.execute(f"UPDATE obs set obs_group_id = {group_id} where person_id = {patient_id} and obs_id > {group_id} and concept_id in (167240, 167237, 167241, 167239) LIMIT 6")
                    connection.commit()
